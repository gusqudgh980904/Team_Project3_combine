...
$(function() {
 ...
 function imgUploadCancel(){
		if(confirm("기존에 있는 이미지는 삭제됩니다. 재업로드 하시겠습니까?")==true){
	    ...
	    $("#imgUploadCancel1").attr("onclick","imgUpload()"); //onclick 추가
	    $("#imgUploadCancel1").prop("id","imgUpload1"); // 아이디변경
...
function imgUpload(){
  ...
  $("#imgFrm").submit();
  $("#imgUpload1").val("업로드취소하기"); //value변경
  $("#imgUpload1").attr("class","btn btn-danger"); // 클래스변경
  $("#imgUpload1").attr("onclick","imgUploadCancel()"); //onclick 추가
  $("#imgUpload1").attr("id","imgUploadCancel1"); // 아이디변경
        ...
        <div class="container">
        	...
        	<h2>게시글 작성</h2>
	        	...
	        	<div style = "float:left; width : 50%; ">
	        	  <form action = "uploadCancel.do" method = "post" enctype="multipart/form-data" id="imgFrmCancel" target="param">
     				<input type ="hidden" value="${detailLb.lb_posts_num }" name = "hid"/>		
      				  <div class="preview" id="preview" style = "border : 1px solid #dfdfdf;  border-radius: 10px;width : 370px; height : 500px; margin-left:45px;  postion:relative; z-index:0">
	        				...
	        				<img style="width: 100px; height: 200px" id="preview-image${i }" src="http://localhost/ss/resources/upload/${img}">
	    				
	    				</c:forEach>
	    			 </div>
				
					<div id = "addInput" style="width : 50%; float : left; padding-left: 50px">
						<input type="file" id="upfile1" name ="upfile1" class="lb_upfile" disabled="disabled"/> 
					
					</div>
					<div style = "float:left; width : 50%; ">
						<input type ="button" value ="+" id= "plusBtn" class="btn" disabled="disabled"/>  <input type ="button" value ="-" id= "minusBtn"class="btn" disabled="disabled"/>  <input type="button" id="imgUploadCancel1" value="사진재설정" onclick="imgUploadCancel()"class="btn btn-danger"/>
        	...
        	<form action="updateProcess.do" method="post" name="updateFrm" id="updateFrm">
     				<input type ="hidden" value="${detailLb.lb_posts_num  }" name = "lb_posts_num"/>		
        	<div>
        		작성자 : <input type="text" name="member_id" value="${detailLb.member_id }" readonly="readonly" class="inputBox"/>
        	</div>	
        	<br/>
			<div>
        		<input type="text" class="text" placeholder="제목을 입력해주세요." value="${detailLb.lb_title}" name="lb_title" id="title"/>
			</div>
			<br/>
	        	<div>
        		<textarea id="summernote" name="lb_contents"><c:out value="${detailLb.lb_contents }"/></textarea>
	        	</div>
        	</form>
	        </div>	
        	
        	<div style = "width : 100%; padding-left : 950px; height : 100px; margin-top : 600px;">
        			
        		 <input type="button" value="수정하기" class="btn btn-info" id="write" /> <input type="button" value="취소" class="btn btn-danger" id="cancel" onclick="location.href='write_comment.do?num=${detailLb.lb_posts_num }'"/>
        	</div>
        ...
        <div class="footer-wrap">
            ...
            <div class="footer-content">
                ...
                <ul>
                    ...
                    <li>한독약품빌딩 8층 쌍용교육센터<iframe id="id" name="param" style="width:1px; height:1px"></iframe></li>
