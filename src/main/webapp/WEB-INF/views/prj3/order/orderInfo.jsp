...
<link href="http://localhost/ss/WebContent/common/css/main.css" rel="stylesheet" >
     ...
     <!-- container start -->
        ...
        <table id="order">
 		 ...
 		 <c:forEach var="order" items="${ InfoList }">
			<tr>
				<td><img src="http://localhost/ss/common/images/product_photo/<c:out value="${ order.prod_image }"/>" width="150" height="150"/></td>
				<td ><c:out value="${ order.prod_name }"/></td>
				<td ><c:out value="${ order.order_price }"/></td>
				<td ><c:out value="${ order.order_size }"/></td>
				<td ><c:out value="${ order.order_quantity }"/></td>
				<td ><c:out value="${ order.prod_num }"/></td>
				<td ><c:out value="${ order.order_price *  order.order_quantity}"/></td>
				
			</tr>   
		</c:forEach>
		</table>
			<div style="padding-top : 30px; padding-left: 450px">
        		<input type="button" value="계속 쇼핑하기" class="btn btn-success" onclick="location.href='http://localhost/ss/main/main_all.do'"/>
			</div>
